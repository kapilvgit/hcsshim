...
func main() {
	...
	err := func() (err error) {
		...
		policyCode, err := securitypolicy.MarshalPolicy(*outputType, config.AllowAll, policyContainers, config.ExternalProcesses, config.AllowPropertiesAccess, config.AllowDumpStacks, config.AllowProcessLogging)
		if err != nil {
			return err
		}
