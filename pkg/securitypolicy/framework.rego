...
# test if a defice hash exists as a layer in a fragment container
deviceHash_ok {
...
default plan9_mount_matches := false

# test if there is a matching plan9 mount in the policy
plan9_mount_matches {
    some p9mount in data.policy.plan9_mounts
    input.target == p9mount
}

# test if there is a matching plan9 mount in a fragment
plan9_mount_matches {
    some issuer in data.metadata.issuers
    some feed in issuer.feeds
    some p9mount in feed.plan9_mounts
    input.target == p9mount
}
